...
function initialize(next){
	...
	var sourceController = [__dirname, '../default/IndexController.js'].join(path.sep);
	...
	//create default view directory
	var viewPath = [configuration.path.scripts, 'index'].join(path.sep);
	if(!fs.existsSync(viewPath))
		fs.mkdirSync(viewPath);

	//create default view
	var sourceView = [__dirname, '../default/index.handlebars'].join(path.sep);
...
function controllerAction(request, response, controllerClass, routing){
...
//handles all controller related requests
function controllerHelper(request, response){
	...
	var index = 0;
	var controllerPath = parts[index++].toLowerCase();
	controllerPath = controllerPath.length ? controllerPath : 'index';
	var actionPath = parts.length > 1 ? parts[index++] : 'index';

	var routing = {
		controller: {
			path: controllerPath,
			name: pascalCase(controllerPath + '-controller')
		},
		action: {
			path: actionPath,
			name: camelCase(actionPath + '-action')
		}
	};
	...
	try{
		...
		if(!controllerClass){
			var content = fs.readFileSync([configuration.path.controllers, routing.controller.name + '.js'].join(path.sep), configuration.encoding);
			var controllerClass = eval(content);
			cache[filename] = controllerClass;
		}

		controllerAction(request, response, controllerClass, routing);
	}
	catch(ex){}
}
