...
var Routing = {
	...
	onRequest: function(request, response){
		...
		configuration.verbose.route(`Route: Route received ${request._parsedUrl.path}.`);
		...
		configuration.verbose.route(`Route: Forwarding route ${request._parsedUrl.path} to controller "${routing.controller.name}" action "${routing.action.name}".`);
	...
	forward: function(request, response, controllerClass, routing){
		try{
			...
			if(!controllerClass)
				throw new Error(`Controller "${routing.controller.name}" could not be loaded.`);
			...
			if(!controller[routing.action.name])
				throw new Error(`Action "${routing.action.name}" not found in controller "${routing.controller.name}"(${[configuration.path.controllers, routing.controller.name + '.js'].join(path.sep)}).`);
            ...
            try{
                ...
                ex.message = `Error in ${routing.controller.name} at ${routing.action.name}: ${ex.message}`;
                throw ex;
            }
	...
	onCustomRoute: function(request, response){
		...
		configuration.verbose.route(`Route: Custom route received ${request._parsedUrl.path}.`);
		...
		//handle custom routes
		var customRoute = Routing.routes.filter(function(route){
			var re = pathToRegexp(route.route);
			return (re.exec(request._parsedUrl.path));
		});

		if(customRoute.length){
			...
			request._parsedUrl.pathname = ['', request._zenhance.route.controller, request._zenhance.route.action].join(path.sep);
		}
...
function initialize(next){
	...
	configuration.path.public = ['public'].join(path.sep);

	//set application directory
	configuration.path.application = ['application'].join(path.sep);

	//set library directory
	configuration.path.library = ['library'].join(path.sep);
	...
	var applicationJSON = [configuration.path.root, configuration.path.configs, 'application.json'].join(path.sep);

	//set routes json file
	var routesJSON = [configuration.path.root, configuration.path.configs, 'routes.json'].join(path.sep);

	//set bootstrap file
	var bootstrapFile = [configuration.path.root, configuration.path.application, 'Bootstrap.js'].join(path.sep);
	...
	function _readConfigFiles(){
		...
		//setup verbose modes
		configuration.verbose.route = configuration.verbose.route ? verbose : function(){};
		configuration.verbose.cache = configuration.verbose.cache ? verbose : function(){};
	...
	if(!fs.existsSync([configuration.path.root, 'application'].join(path.sep))){
		...
		i.question('It appears that this is the first time you are running Zenhance or required files are missing. Would you like Zenhance to create a barebones application for you and/or restore missing required files? (Y/n): ', function(answer){
			...
			if(answer.toLowerCase() == 'y' || !answer){
				...
				var dir = [configuration.path.root, configuration.path.public].join(path.sep);
				...
				var dir = [configuration.path.root, configuration.path.library].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);

				//create the configs directory if it doesn't exist
				var dir = [configuration.path.root, configuration.path.configs].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);

				//create the controllers directory if it doesn't exist
				var dir = [configuration.path.root, configuration.path.controllers].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);

				//create the models directory if it doesn't exist
				var dir = [configuration.path.root, configuration.path.models].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);

				//create the views directory if it doesn't exist
				var dir = [configuration.path.root, configuration.path.views].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);

				//create the layouts directory if it doesn't exist
				var dir = [configuration.path.root, configuration.path.layouts].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);
				...
				var dir = [configuration.path.root, configuration.path.scripts].join(path.sep);
				if(!fs.existsSync(dir))
					fs.mkdirSync(dir);

				//create default index controller
				var sourceController = [__dirname, '..', 'default', 'controllers', 'IndexController.js'].join(path.sep);
				var destinationController = [configuration.path.root, configuration.path.controllers, 'IndexController.js'].join(path.sep);
				...
				var destinationController = [configuration.path.root, configuration.path.controllers, 'ErrorController.js'].join(path.sep);
				if(!fs.existsSync(destinationController))
					fs.createReadStream(sourceController).pipe(fs.createWriteStream(destinationController));
...
//method for handling verbose output
...
var Zenhance = {
	...
	run: function(){
		initialize(function(){
			...
			fs.watch(configuration.path.root, {recursive: true}, function(event, filename){
				...
				configuration.verbose.cache(`Cache: File change notification for "${fullname}"`);
	...
	script: function(configuredPath, resourceName){
		...
		object = require.cache[scriptFile.trim()];

		console.info(`Cache: Cached file ${scriptFile} exists.`, object != null);
		...
		if(!object){
			...
			configuration.verbose.cache(`Cache: Caching script file ${scriptFile}`);

			require.cache[scriptFile.trim()] = object;
		}
	...
	controller: function(name, moduleName){
		...
		try{
			if(moduleName){
				var dir = [configuration.path.root, configuration.path.application, 'modules', moduleName, 'controllers'].join(path.sep);
				controller = Zenhance.script(dir, name);
			}
			else{
				var dir = [configuration.path.root, configuration.path.controllers].join(path.sep);
				controller = Zenhance.script(dir, name);
	...
	library: function(name){
		...
		try{
			var dir = [configuration.path.root, configuration.path.library].join(path.sep);
			library = Zenhance.script(dir, name);
		}
		catch(ex){
			publishError(ex);
		}
	...
	model: function(name){
		...
		try{
			var dir = [configuration.path.root, configuration.path.models].join(path.sep);
			model = Zenhance.script(dir, name);
		}
		catch(ex){
			publishError(ex);
		}
