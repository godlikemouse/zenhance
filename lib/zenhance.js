	...
	helpers: {
		...
		headLink: function() {
			return {
				...
				setStylesheet: function(href, media, conditionalStylesheet, attrs){
...
var Routing = {
	...
	forward: function(request, response, controllerClass, routing){
		try{
			...
			//reset head script/link
			Registry._headScript = Registry._headLink = {};
			...
			if(!controller.disableRenderer)
				...
				controller.view.headScript = configuration.template.safeString(Registry.helpers.headScript().toString());
				controller.view.headLink = configuration.template.safeString(Registry.helpers.headLink().toString());
